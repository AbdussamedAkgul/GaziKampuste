...
start_date = date(2025, 9, 22) # Güz dönemi başlangıcı (Pazartesi)
end_date = date(2025, 12, 29) # Güz dönemi sonu
...
with app.app_context():
    ...
    if not admin_user:
            ...
            for day_idx, time_str, class_name in classes:
                if day_idx == weekday:
                    ...
                    existing = db.session.scalar(
                        db.select(Event).where(Event.title == title_str, Event.start_date == current_date)
                    )
                    if not existing:
                        ev = Event(
                            title=title_str,
                            description="2025-2026 Güz Dönemi Ders Programı",
                            start_date=current_date,
                            end_date=None,
                            event_type="school", # Okul etkinliği kategorisinde eklenecek
                            author=admin_user
                        )
                        db.session.add(ev)
        ...
        db.session.commit()
        print("Ders programı başarıyla takvime işlendi!")
