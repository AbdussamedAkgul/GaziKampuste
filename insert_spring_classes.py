...
start_date = date(2026, 2, 9) # Bahar dönemi başlangıcı (Pazartesi)
end_date = date(2026, 5, 22) # Bahar dönemi sonu
...
with app.app_context():
    ...
    if not admin_user:
            ...
            for day_idx, time_str, class_name in classes:
                if day_idx == weekday:
                    ...
                    if not existing:
                        ev = Event(
                            title=title_str,
                            description="2025-2026 Bahar Dönemi Ders Programı",
        ...
        db.session.commit()
        print("2026 Bahar ders programı başarıyla takvime işlendi!")
